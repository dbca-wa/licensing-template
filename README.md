...
Then, run `npm run build` to build the software and move the output files to `licensing_template/static/licensing_template_vue`.

The build files are made available to the Django app by running `./collectstatic.sh`.

If the `DEV_APP_BUILD_URL` is not set, the Django app will serve static Javascript from `staticfiles/licensing_template_vue/js.app.js`, 
...
required environment variables at run time. Example content:
    ...
    DEV_APP_BUILD_URL="http://localhost:8080/static/licensing_template_vue/js/app.js"
