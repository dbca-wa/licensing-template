...
from licensing_template.components.main.models import RevisionedMixin
...
from licensing_template.components.approvals.email import (
...
from licensing_template.settings import PROPOSAL_TYPE_AMENDMENT, PROPOSAL_TYPE_RENEWAL
from licensing_template.utils import search_keys, search_multiple_keys
from licensing_template.helpers import is_customer

# from licensing_template.components.approvals.email import send_referral_email_notification
...
class Approval(RevisionedMixin):
    ...
    # submitter = models.ForeignKey(EmailUser, on_delete=models.PROTECT, blank=True, null=True, related_name='licensing_template_approvals')
    ...
    def generate_doc(self, user, preview=False):
        from licensing_template.components.approvals.pdf import (
            create_approval_doc,
    ...
    #        from licensing_template.components.approvals.pdf import create_approval_pdf_bytes
    ...
    def generate_renewal_doc(self):
        from licensing_template.components.approvals.pdf import create_renewal_doc
    ...
    def approval_surrender(self, request, details):
        with transaction.atomic():
            try:
                if not request.user.licensing_template_organisations.filter(
