...
export default {
    ...
    computed: {
        ...
        hasOrgs: function() {
            return this.profile.licensing_template_organisations && this.profile.licensing_template_organisations.length > 0 ? true: false;
        },
    ...
    methods: {
            ...
            vm.$http.post(helpers.add_endpoint_json(api_endpoints.organisations,(vm.newOrg.id+'/validate_pins')),JSON.stringify(this.newOrg),{
                ...
                if (response.body.valid){
                    ...
                    this.$http.get(api_endpoints.profile).then((response) => {
                        ...
                        if (vm.profile.residential_address == null){ vm.profile.residential_address = {}; }
                        if (vm.profile.postal_address == null){ vm.profile.postal_address = {}; }
                        if ( vm.profile.licensing_template_organisations && vm.profile.licensing_template_organisations.length > 0 ) { vm.managesOrg = 'Yes' }
                    },(error) => {
                        console.log(error);
                    })
                }else {
        ...
        orgConsultRequest: function() {
            ...
            for (var organisation in vm.profile.licensing_template_organisations) {
                if (new_organisation.abn && vm.profile.licensing_template_organisations[organisation].abn == new_organisation.abn) {
                    swal({
